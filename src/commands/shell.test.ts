...
vi.mock("../util/shell-reader", () => ({
  ShellReader: vi.fn(function ShellReader(
    this: unknown,
    code: string,
    options: unknown,
  ) {
...
describe("jsk sh — final output rendering", () => {
  ...
  it("keeps codeblock fences on every page, both the initial (tail) page and after paging", async () => {
    ...
    const collector = sentMessage.createReactionCollector.mock.results[0]
    ...
    const lastEditCall = sentMessage.edit.mock.calls.at(-1) as [
      { content: string },
    ];
  ...
  it("passes the configured shell override through to ShellReader", async () => {
    ...
    const [, options] = vi.mocked(ShellReader).mock.calls.at(-1) as [
      string,
      { shell?: unknown },
    ];
  ...
  it("does not send a duplicate message when a periodic flush is still in flight when the command finishes", async () => {
    ...
    try {
      ...
      const send = vi.fn((): any =>
        send.mock.calls.length === 1 ? firstSend : sentMessage,
      );
      // biome-ignore lint/suspicious/noExplicitAny: minimal fake message for tests.
      const message = { channel: { send }, author: { id: "owner-1" } } as any;
...
describe("jsk sh — shellOwners permission tier", () => {
  it("rejects a general owner who is not in shellOwners", async () => {
    stubReader(["hi"]);
    const jsk = makeJsk({ owners: ["owner-1"], shellOwners: ["someone-else"] });
    const { ctx, send } = makeContext("echo hi", jsk);

    await shellCommand.handler(ctx);

    // If the shell had actually run, send() would've received command
    // output instead of this exact rejection string.
    expect(send).toHaveBeenCalledWith(
      "❌ You are not permitted to use shell commands.",
    );
  });

  it("allows a user in shellOwners even when shellOwners is narrower than owners", async () => {
    stubReader(["hi"]);
    const jsk = makeJsk({
  ...
  it("falls back to owners when shellOwners is not configured", async () => {
    stubReader(["hi"]);
    const jsk = makeJsk({ owners: ["owner-1"] });
    const { ctx, send } = makeContext("echo hi", jsk);
