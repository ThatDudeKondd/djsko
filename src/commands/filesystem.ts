  ...
  async handler(ctx) {
    ...
    const lineSpan = match[2]
      ? ([
          ...
          Number.parseInt(match[3] ?? match[2], 10),
        ] as const)
      : null;

    if (lineSpan && (lineSpan[0] < 1 || lineSpan[1] < lineSpan[0])) {
      await ctx.send(
        "Line numbers must start at 1, and the range end must not precede its start.",
      );
      return;
    }
...
const curlCommand: Command = {
  ...
  async handler(ctx) {
    ...
    if (!data) {
      await ctx.send(
        `HTTP response was empty (status code ${response.status}).`,
      );
      return;
    }
