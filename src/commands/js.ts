...
const retainCommand: Command = {
  ...
  async handler(ctx) {
    ...
    if (toggle === undefined) {
      await ctx.send(
        `Variable retention is set to ${jsk.retain ? "ON" : "OFF"}.`,
      );
      return;
    }
    ...
    if (ENABLE.has(toggle)) {
      ...
      jsk.lastResult = null;
      await ctx.send(
        "Variable retention is ON. Assign to `vars` to persist across REPL sessions.",
      );
      return;
    }
    ...
    if (DISABLE.has(toggle)) {
      ...
      jsk.retain = false;
      await ctx.send(
        "Variable retention is OFF. Future REPL sessions will not retain their scope.",
      );
      return;
    }

    await ctx.send(
      "Provide `on` or `off` (or nothing to see the current status).",
    );
  },
};
