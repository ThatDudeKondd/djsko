...
export async function statusCommand(ctx: Context): Promise<void> {
  ...
  if (shard?.ids) {
    summary.push(
      `This bot is sharded (shard ${shard.ids.join(", ")} of ${shard.count}).`,
    );
  }
...
const helpCommand: Command = {
  ...
  async handler(ctx) {
    ...
    for (const command of COMMANDS) {
      ...
      lines.push(
        `\`${ctx.prefix}jsk ${command.name}${usage}\`${aliases} — ${command.summary}`,
      );
    }
    await ctx.send({
      content: lines.join("\n"),
...
const tasksCommand: Command = {
  ...
  async handler(ctx) {
    ...
    const lines = tasks.map(
      (task) =>
        `${task.index}: \`${task.command}\`, invoked at ${task.invokedAt.toISOString()}`,
    );
...
const cancelCommand: Command = {
  ...
  async handler(ctx) {
    ...
    if (Number.isNaN(index)) {
      await ctx.send(
        "Provide a task index, `-1` for the most recent, or `~` for all.",
      );
      return;
    }
