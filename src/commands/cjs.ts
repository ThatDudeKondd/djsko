...
const cjsCommand: Command = {
  ...
  aliases: ["commonjs"],
  summary:
    "Evaluates JavaScript with `require` available. Use `return` to produce a result.",
  async handler(ctx) {
    ...
    const realRequire = createRequire(
      path.join(ctx.jsk.config.evalModuleDir, "jsk-eval-shim.cjs"),
    );
    ...
    const require = createGuardedRequire(
      realRequire,
      ctx.jsk.config.evalTimeout,
    );
