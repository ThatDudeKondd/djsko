...
const shellCommand: Command = {
  ...
  async handler(ctx) {
    ...
    if (!(await ctx.jsk.shellOwners.isOwner(ctx.author.id))) {
      await ctx.send("❌ You are not permitted to use shell commands.");
      return;
    }
    ...
    const renderTail = (): string => {
      const tail =
        output.length > TAIL_LIMIT
          ? output.slice(output.length - TAIL_LIMIT)
          : output;
    ...
    const headerCode =
      code.length > MAX_HEADER_CODE_LENGTH
        ? `${code.slice(0, MAX_HEADER_CODE_LENGTH)}…`
        : code;
