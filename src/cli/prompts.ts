...
export async function promptBotOrSelfbot(): Promise<"bot" | "selfbot"> {
  const value = await consola.prompt(
    "Are you setting up a bot or a selfbot (user account)?",
    {
      type: "select",
      options: [
        { label: "Bot", value: "bot" },
...
export async function promptOwners(): Promise<string[]> {
  const raw = await consola.prompt(
    "Owner user ID(s), space-separated (leave empty to skip):",
    {
      type: "text",
      default: "",
      cancel: "reject",
...
export async function promptSelfbotLibrary(): Promise<SelfbotLibrary> {
  const value = await consola.prompt("Selfbot library:", {
    ...
    options: [
      ...
      {
        label: "discord.js-selfbot-youtsuho-v13",
        value: "discord.js-selfbot-youtsuho-v13",
      },
    ],
    cancel: "reject",
  });
