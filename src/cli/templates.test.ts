...
function makeSelfbotAnswers(
  overrides: Partial<SelfbotAnswers> = {},
): SelfbotAnswers {
  return {
    kind: "selfbot",
...
describe("buildPackageJson", () => {
  it("includes djsk and the resolved discord.js range for a bot", () => {
    const pkg = buildPackageJson(
      makeBotAnswers({ discordJsRange: "^14.26.4" }),
      "0.1.0",
    );
  ...
  it("adds TS devDependencies and tsx-based scripts only in TS mode", () => {
    const jsPkg = buildPackageJson(
      makeBotAnswers({ format: "js" }),
      "0.1.0",
    ) as {
    ...
    const tsPkg = buildPackageJson(
      makeBotAnswers({ format: "ts" }),
      "0.1.0",
    ) as {
      scripts: Record<string, string>;
  ...
  it("adds a deploy script only when the bot mode includes slash, using the right runner", () => {
    const textOnly = buildPackageJson(
      makeBotAnswers({ commandMode: "text" }),
      "0.1.0",
    ) as {
      scripts: Record<string, string>;
    };
...
describe("buildTsconfig", () => {
  it("targets ES2022 with strict mode and noEmit", () => {
    const tsconfig = buildTsconfig() as {
...
describe("buildBotEntry", () => {
  ...
  it("wires only interactionCreate and uses minimal intents for slash-only", () => {
    const entry = buildBotEntry(
      makeBotAnswers({ commandMode: "slash", discordVersion: "v14" }),
    );
  ...
  it("always includes GuildMessageReactions (needed for output pagination)", () => {
    const v14Slash = buildBotEntry(
      makeBotAnswers({ discordVersion: "v14", commandMode: "slash" }),
    );
    expect(v14Slash).toContain("GatewayIntentBits.GuildMessageReactions");

    const v14Text = buildBotEntry(
      makeBotAnswers({ discordVersion: "v14", commandMode: "text" }),
    );
    expect(v14Text).toContain("GatewayIntentBits.GuildMessageReactions");

    const v13Slash = buildBotEntry(
      makeBotAnswers({ discordVersion: "v13", commandMode: "slash" }),
    );
    expect(v13Slash).toContain("Intents.FLAGS.GUILD_MESSAGE_REACTIONS");

    const v13Text = buildBotEntry(
      makeBotAnswers({ discordVersion: "v13", commandMode: "text" }),
    );
  ...
  it("uses GatewayIntentBits for v14 and Intents.FLAGS for v13", () => {
    const v14 = buildBotEntry(
      makeBotAnswers({ discordVersion: "v14", commandMode: "text" }),
    );
    expect(v14).toContain("GatewayIntentBits");

    const v13 = buildBotEntry(
      makeBotAnswers({ discordVersion: "v13", commandMode: "text" }),
    );
  ...
  it("includes explicit owners when provided, and a placeholder comment otherwise", () => {
    const withOwners = buildBotEntry(
      makeBotAnswers({ owners: ["111", "222"] }),
    );
  ...
  it("includes security: true only when security mode is enabled", () => {
    expect(buildBotEntry(makeBotAnswers({ security: true }))).toContain(
      "security: true,",
    );
    expect(buildBotEntry(makeBotAnswers({ security: false }))).not.toContain(
      "security: true,",
    );
  });
...
describe("buildSelfbotEntry", () => {
  ...
  it("includes a custom prefix only when it differs from the default", () => {
    ...
    const defaultPrefix = buildSelfbotEntry(
      makeSelfbotAnswers({ prefix: "." }),
    );
    expect(defaultPrefix).not.toMatch(/^\s*prefix:/m);
  });
});
