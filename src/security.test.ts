...
describe("SecretScrubber", () => {
  it("redacts the client token", () => {
    expect(scrubber.scrub("my token is super-secret-bot-token!")).toBe(
  ...
  it("redacts secret-like process.env values", () => {
    ...
    expect(s.scrub("leaked value=abcdef123456 here")).toBe(
      "leaked value=[redacted] here",
    );
    delete process.env.MY_TEST_API_SECRET;
  });

  it("redacts .env-style assignment lines", () => {
    expect(scrubber.scrub("DISCORD_TOKEN=abc.def.ghi")).toBe(
      "DISCORD_TOKEN=[redacted]",
    );
    expect(scrubber.scrub('export API_KEY="xyz12345"')).toBe(
  ...
  it("redacts Discord webhook URLs", () => {
    const url =
      "https://discord.com/api/webhooks/123456789012345678/abcDEF-ghi_jkl";
    expect(scrubber.scrub(`hook: ${url}`)).toBe("hook: [redacted]");
  });

  it("leaves ordinary text untouched", () => {
    expect(scrubber.scrub("hello world 1 + 1 = 2")).toBe(
      "hello world 1 + 1 = 2",
    );
  });
