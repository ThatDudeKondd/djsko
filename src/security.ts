...
export class SecretScrubber {
  ...
  private literals(): string[] {
    ...
    for (const [key, value] of Object.entries(process.env)) {
      if (value && value.length >= MIN_SECRET_LENGTH && SECRET_KEY.test(key))
        out.push(value);
    }
...
export function scrubMessagePayload(
  ...
  const out = Object.assign(
    Object.create(Object.getPrototypeOf(payload)),
    payload,
  );
  ...
  if (
    ...
    typeof out.options.content === "string"
  ) {
  ...
  if (scrubFiles && Array.isArray(out.files)) {
    ...
    out.files = out.files.map((file: any) => {
      ...
      if (Buffer.isBuffer(attachment) || typeof attachment === "string") {
        const text = Buffer.isBuffer(attachment)
          ? attachment.toString("utf-8")
          : attachment;
 ...
 */
export function guardOutbound<T extends object>(
  target: T,
  scrub: (text: string) => string,
): T {
